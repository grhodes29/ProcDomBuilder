...
public class CSharpClassWriter implements IClassWriter {
...
public CSharpClassWriter(String iAbsoluteClassDirectoryLocation, ProcBuilder lProcBuilder, String iAuthorName, DatabaseServerType iDatabaseServerType){
		
		this._MSSQLProcBuilder = (MSSQLProcBuilder)lProcBuilder;
		this._AbsoluteClassDirectoryLocation = iAbsoluteClassDirectoryLocation;
		this._author = iAuthorName;
...
public void CreateClassFields(List<MetaRow> iFieldList){
	...
	for (int rowcounter = 0; rowcounter < iFieldList.size(); rowcounter++){
		...
		if (this._dbservertype == DatabaseServerType.MICROSOFTSQLSERVER){
			lDataMemberType = this.MSSQLServerDbTypeToCSharpType(lMetaRow.get_data_type());
		...
		lClassFields = lClassFields + "private " + lDataMemberType.toLowerCase() + " _" + lDataMemberName + ";\n";
...
public void CreateClassGettersSetters(List<MetaRow> iFieldList){
		...
		for (int rowcounter = 0; rowcounter < iFieldList.size(); rowcounter++){
			...
			lGettersSetters = lGettersSetters + "public " + lDataMemberType.toLowerCase() + " _" + lDataMemberName + "\n";
			lGettersSetters = lGettersSetters + "{\n";
			lGettersSetters = lGettersSetters + "\tget { return _" + lDataMemberName + "; }\n";
			lGettersSetters = lGettersSetters + "\tset { _" + lDataMemberName + " = value; }\n";
			lGettersSetters = lGettersSetters + "}\n\n";
		}
...
public void CreateClassConstructor(List<MetaRow> iFieldList) {
	...
	for (int rowcounter = 0; rowcounter < iFieldList.size(); rowcounter++){
		...
		lParameters = lParameters + lDataMemberType.toLowerCase() + " i" + lDataMemberName + ", ";
		}
		else{			
			lParameters = lParameters + lDataMemberType.toLowerCase() + " i" + lDataMemberName + ")\n";
		}
...
public void ClassFileWriter(String iClassDirectoryLocation, String iClass, String iClassName)
{
	...
	PrintWriter writer;
	try {
		lFile = new File(iClassDirectoryLocation + "/" + iClassName + ".cs");
...
private String MSSQLServerDbTypeToCSharpType(String iDataType){
	String lReturn = "";
